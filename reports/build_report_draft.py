...
def build():
    ...
    r.p(
        ...
        "together they answer the brief's roadmap question. Section 11 gathers the final "
        "model in one place, section 12 covers the difficulties, section 13 what we "
        "learned beyond the course, and section 14 the limitations we did not resolve."
    )
    r.p(
    ...
    r.table(
        ...
        [["Move off the default cut to a global predicted share", "7", "+0.07845"],
         ...
         ["Hyperparameter tuning", "5", "not measurable"]],
        "Where the score came from. Roughly 70% of the total came from how scores are "
        "turned into labels, 30% from what the model sees, and nothing measurable from "
        "the model itself.",
        widths=[3.2, 0.8, 2.1],
    )
    r.p(
        "That ordering is the honest summary of the project and it is not the one we "
        "expected. Two full phases spent on the model, tuning and ensembling, returned "
        "gains at or below what the leaderboard can resolve, and we report them as the "
        "nulls they were rather than dressing them up. The two phases that worked were "
        "the ones that questioned the decision rule and the input."
    )
    r.p(
    ...
    # ------------------------------------------------------------------ 2
    r.h("2. Phase 1: understand the data before modelling it")
    r.p(
    ...
    r.figure("text_length_distribution.png",
             ...
             "basis of our transfer-validation protocol in section 12.2.", width=5.8)
    ...
    r.h("3.1 Task 1 deliverable: logistic regression from scratch", level=2)
    r.p(
    ...
    r.todo("batch size, epochs and learning rate used, the validation Macro F1, and the "
           "same figure from sklearn's LogisticRegression for the side-by-side comparison "
           "the rubric asks for")
    r.todo("the prediction file. The course brief slide names it LogReg_predictions.csv "
           "while the written task sheet names it LogReg_Prediction.csv. Confirm the "
           "spelling against the Kaggle submission page before generating it, since the "
           "file does not exist yet")

    r.h("3.2 Task 2 deliverable: PCA and KNN at four component counts", level=2)
    r.p(
    ...
    r.p(
        "That also predicts the KNN result. Reducing to 100 components throws away 84% of "
        "the variance, and KNN then measures Euclidean distance in a space where most of "
        "what distinguished the documents has been discarded. Neighbourhoods in a sparse "
        "high-dimensional space are weakly defined to begin with, which is why KNN came "
        "tenth of twelve in phase 4 even at full width."
    )
    ...
    r.todo("the four test Macro F1 scores read back from Kaggle at 2000, 1000, 500 and "
           "100 components, which is the graded deliverable for Task 2")

    r.h("3.3 The decision this phase settled for Task 3", level=2)
    r.considered(
        [
    ...
    # ------------------------------------------------------------------ 4
    r.h("4. Phase 3: lock the validation protocol")
    r.p(
    ...
    # ------------------------------------------------------------------ 5
    r.h("5. Phase 4: baselines before any tuning")
    r.p(
    ...
    # ------------------------------------------------------------------ 6
    r.h("6. Phase 5: tuning, and a first leaderboard result that went badly")
    r.p(
    ...
    # ------------------------------------------------------------------ 7
    r.h("7. Phase 6: ensembling, and an honest null")
    r.p(
    ...
    # ------------------------------------------------------------------ 8
    r.h("8. Phase 7: threshold calibration")
    r.p(
    ...
    # ------------------------------------------------------------------ 9
    r.h("9. Phase 8: the pivot to raw-text features")
    r.p(
    ...
    r.p(
        ...
        f"leaving the {facts.BEST_N_FEATURES:,}-column set in section 11. Note also that "
        "the diversity block returns more than character n-grams from five columns against "
        "20,000, which is what prompted the expansion attempt reported in section 12.4."
    )
    r.considered(
        [
    ...
    r.decision(
        ...
        because="it was worth +0.044 on the graded surface, which is more than everything "
                ...
                "provided it is described in full, and this section plus section 11 is "
                "that description.",
    )
    ...
    # ------------------------------------------------------------------ 10
    r.h("10. Phase 9: calibration revisited, per group")
    r.p(
    ...
    r.h("10.1 Hyperparameters on the new representation", level=2)
    r.p(
        "One thread is still open and we report it as open. Every hyperparameter search in "
        "this project ran against the supplied 5,000 features. Nothing has been tuned on "
        f"the {facts.BEST_N_FEATURES:,}-column raw-text matrix, which is a different "
        "problem in shape: eight times wider, sparser, and three quarters character "
        "n-grams. The shipped model therefore runs on LightGBM defaults with only "
        "class_weight set."
    )
    r.p(
        "The search is designed and split five ways across the team over 400 seeded "
        "configurations, sampling learning rate, tree count, leaf count, depth, minimum "
        "child samples, column and row subsampling and both regularisation terms. It "
        "selects on paired per-fold differences against the default configuration rather "
        "than on raw means, requires a candidate to improve on every fold, and confirms "
        "the winner on a protocol the search never selected on. Based on the equivalent "
        "search on the supplied features, we expect it to be worth about 0.005."
    )
    r.todo("result of the notebook 17 hyperparameter search once the team has run it, "
           "including the winning configuration and its paired per-fold margin")

    # ------------------------------------------------------------------ 11
    r.h("11. The final model, end to end")
    r.p(
        "This section answers the brief's first question in one place, gathering what the "
        "nine phases arrived at."
    )
    r.h("11.1 The classifier", level=2)
    r.p(
    ...
    r.p(
        ...
        "human class be outvoted. Every other hyperparameter is at its default, for the "
        "reason given in section 10.1."
    )
    r.h("11.2 The features", level=2)
    r.table(
    ...
    r.h("11.3 The decision rule", level=2)
    r.p(
    ...
    r.h("11.4 What it scores", level=2)
    r.table(
    ...
    # ------------------------------------------------------------------ 12
    r.h("12. Difficulties, and what each one cost")
    r.p(
    ...
    r.h("12.1 We compared models along an axis we had not held fixed", level=2)
    r.p(
    ...
    r.p(
        "The inversion was an artifact of the confound described in section 8. Once we "
    ...
    r.h("12.2 A validation protocol that would not reproduce", level=2)
    r.p(
    ...
    r.h("12.3 A diagnostic that silently contaminated itself", level=2)
    r.p(
    ...
    r.h("12.4 Two predictions we made, and both were wrong", level=2)
    r.p(
    ...
    r.h("12.5 What we could not overcome", level=2)
    r.bullets([
    ...
    # ------------------------------------------------------------------ 13
    r.h("13. What we learned beyond the course")
    r.h("13.1 Averaging can hide two large effects that cancel", level=2)
    r.p(
    ...
    r.h("13.2 Blocked and paired comparison", level=2)
    r.p(
    ...
    r.h("13.3 Leave-one-group-out validation as a transfer proxy", level=2)
    r.p(
        ...
        "control in section 12.2: without it, a lower score under grouping is equally "
        "consistent with the protocol simply training on fewer rows, and the whole "
        "apparatus proves nothing."
    )
    r.h("13.4 A leaderboard is a measuring instrument with a precision", level=2)
    r.p(
    ...
    r.h("13.5 Reading the dataset paper was worth more than any model", level=2)
    r.p(
    ...
    # ------------------------------------------------------------------ 14
    r.h("14. Limitations and next steps")
